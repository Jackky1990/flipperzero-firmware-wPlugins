#!/usr/bin/env python3
import sys
from commands import doctor, check_api, check_deps, build, status, verify, test, contract
...
def main():
    if len(sys.argv) < 2:
        print("usage: aep.py [doctor|check-api|check-deps|build|status|test|contract|verify]")
        return 2
