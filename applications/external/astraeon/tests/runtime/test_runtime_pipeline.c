...
static bool pipeline_handler_called = false;

static AstraResult astra_test_pipeline_handler(AstraEvent* event) {
    if(!event) {
        return astra_result_error(AstraStatusInvalidArgument, "event is null");
    }

    pipeline_handler_called = true;
    return astra_result_ok();
}

bool astra_test_runtime_pipeline(void) {
    AstraEvent event;
    pipeline_handler_called = false;

    if(astra_registry_init().status != AstraStatusOk) return false;
    if(astra_event_bus_init().status != AstraStatusOk) return false;

    if(astra_registry_register(AstraEventTypeStatus, astra_test_pipeline_handler).status != AstraStatusOk) {
        return false;
    }
    ...
    return pipeline_handler_called && event.state == AstraEventStateCompleted;
}
